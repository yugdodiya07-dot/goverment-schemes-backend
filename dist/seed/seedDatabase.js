...
const schemesData = [
    ...
    },
    {
        ...
        description: 'PMAY aims to provide housing for all eligible urban and rural families. Beneficiaries receive credit-linked interest subsidies and direct financial transfers to construct or purchase their first pucca home.',
        benefitType: 'Subsidy',
...
const seedDatabase = async () => {
    try {
        ...
        let admin = await Admin_js_1.Admin.findOne({ email: 'admin@gmail.com' });
        if (!admin) {
            admin = await Admin_js_1.Admin.create({
        ...
        let citizen = await User_js_1.User.findOne({ email: 'citizen@gmail.com' });
        if (!citizen) {
            citizen = await User_js_1.User.create({
                name: 'Aarav Sharma',
        ...
        for (const cat of categoriesData) {
            const existing = await Category_js_1.Category.findOneAndUpdate({ slug: cat.slug }, { ...cat, isActive: true }, { upsert: true, new: true });
        ...
        for (const scheme of schemesData) {
            ...
            if (!categoryId)
                continue;
            await Scheme_js_1.Scheme.findOneAndUpdate({ slug: scheme.slug }, {
                title: scheme.title,
                slug: scheme.slug,
                category: categoryId,
